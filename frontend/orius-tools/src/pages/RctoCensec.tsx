...
import { validarRctoService, corrigirRctoService } from '../services/api';
import type { ApiResponseCensec, InstrucaoCorrecao } from '../types';
import { formatXML } from '../utils/xmlHelpers';

export default function RctoCensec() {
  ...
  const handleValidate = async () => {
    ...
    setLoading(true);
    try {
      const data = await validarRctoService(file);
      setResult(data);
      data.success ? toast.success("XML RCTO 100% válido!") : toast.warning(`${data.erros.length} inconsistências encontradas.`);
    } catch (err) {
      toast.error("Erro na validação do RCTO.");
  ...
  const handleExportarOuRevalidar = async () => {
    ...
    // Mapeamento inteligente de tags baseado na mensagem de erro do rctoService.js
    const instrucoes: InstrucaoCorrecao[] = result.erros
      .filter(erro => correcoesManuais[erro.linhaDoArquivo])
      .map(erro => {
        const msg = erro.mensagemDeErro;
        let campo = 'nome';

        if (msg.includes('tipoTestamento')) campo = 'tipoTestamento';
        else if (msg.includes('CPF')) campo = 'cpf';
        else if (msg.includes('Mãe')) campo = 'nomeMae';
        else if (msg.includes('revogacaoCartorioCns')) campo = 'revogacaoCartorioCns';
        else if (msg.includes('revogacaoDataTestamento')) campo = 'revogacaoDataTestamento';
        else if (msg.includes('revogacaoLivro')) campo = 'revogacaoLivro';
        else if (msg.includes('revogacaoFolha')) campo = 'revogacaoFolha';
        else if (msg.includes('documentoComplemento')) campo = 'documentoComplemento';
    ...
    result.erros.forEach(erro => {
      if (correcoesManuais[erro.linhaDoArquivo]) {
        const instr = instrucoes.find(i => i.linhaDoArquivo === erro.linhaDoArquivo);
        novoHistorico[erro.linhaDoArquivo] = {
          campo: instr?.campo || 'Campo',
    ...
    localStorage.setItem('orius_rcto_history', JSON.stringify(novoHistorico));

    setLoading(true);
    try {
      const response = await corrigirRctoService(file, instrucoes);
      ...
      if (response.success || response.errorCount === 0) {
        ...
        a.href = url;
        a.download = `CORRIGIDO_RCTO_${file.name}`;
        ...
        const data = await validarRctoService(novoArquivo);
        setResult(data);
        setCorrecoesManuais({});
      }
    } catch (err) {
      toast.error("Erro ao processar correções RCTO.");
    } finally {
      setLoading(false);
    }
  };

  // As funções handleCodeChange e handleGoToLine mantêm a paridade com os outros módulos
  const handleCodeChange = (v: string | undefined) => {
    if (v) {
      setXmlCode(v);
      setFile(new File([new Blob([v], { type: 'text/xml' })], file?.name || "rcto.xml", { type: 'text/xml' }));
    }
  };
      ...
      <div className="pb-20 animate-in fade-in duration-500">
        <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-20 shadow-sm px-6 py-4 flex items-center justify-between transition-colors text-gray-900 dark:text-white">
          ...
          <div className="flex gap-2">
            <button onClick={() => { setResult(null); setFile(null); localStorage.removeItem('orius_rcto_history'); }} className="px-3 py-1.5 text-xs font-bold bg-gray-100 dark:bg-gray-700 rounded-lg hover:bg-gray-200 transition-colors">Voltar</button>
            ...
            {!result.success && (
              <>
                ...
                >
                  ...
                  {todosErrosRespondidos ? 'Exportar RCTO' : 'Corrija para Exportar'}
                </button>
              </>
            )}
          </div>
        ...
        <div className="p-6 max-w-7xl mx-auto space-y-6">
          <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <StatsCard title="Testamentos" value={result.total_atos_agrupados} icon={Search} />
          ...
          {result.success ? (
            <div className="space-y-6 animate-in zoom-in-95 duration-500">
              <div className="bg-white dark:bg-gray-800 p-10 rounded-3xl border-2 border-dashed border-green-200 dark:border-green-900/30 flex flex-col items-center justify-center text-center shadow-sm">
                ...
                <h2 className="text-2xl font-black text-gray-900 dark:text-white">RCTO Validado!</h2>
                <p className="text-gray-500 max-w-sm mt-2 text-sm font-medium">Os atos de testamento foram auditados conforme o manual oficial e o download foi liberado.</p>
              ...
              {Object.keys(historicoAcumulado).length > 0 && (
                <div className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 overflow-hidden shadow-sm">
                  <div className="px-6 py-4 border-b border-gray-50 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50 flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-black text-gray-900 dark:text-white uppercase tracking-wider">Log de Alterações - RCTO</h3>
                  </div>
                  <div className="p-0">
                  ...
                  {result.erros.map((erro, idx) => (
                    <tr key={idx} className="hover:bg-gray-50/30 dark:hover:bg-gray-700/10 transition-colors">
                      <td className="px-6 py-4">
                        ...
                        <div className="text-[11px] text-gray-500 truncate max-w-[200px]">{erro.nomeDaParte || 'Testador'}</div>
                      </td>
                      <td className="px-6 py-4">
    ...
    <div className="h-full flex flex-col items-center justify-center p-6 min-h-[calc(100vh-4rem)] bg-gray-50/30 dark:bg-transparent text-gray-900 dark:text-white transition-colors">
      <div className="max-w-xl w-full space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-3xl font-black tracking-tight leading-tight">Módulo <span className="text-orange-500">RCTO</span></h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 font-medium px-8 leading-relaxed">Auditoria instantânea do Registro Central de Testamentos Online conforme as normas da CENSEC.</p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-4">
          <div onDragEnter={(e) => { e.preventDefault(); setDragActive(true); }} onDragLeave={() => setDragActive(false)} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); setDragActive(false); e.dataTransfer.files[0] && processFile(e.dataTransfer.files[0]); }} className={`relative flex flex-col items-center justify-center py-10 px-4 border-2 border-dashed rounded-2xl transition-all group ${dragActive ? 'border-orange-500 bg-orange-50/50 dark:bg-orange-500/5' : 'border-gray-300 dark:border-gray-600 hover:border-orange-400/50'}`}>
            ...
            <h3 className="text-base font-bold tracking-tight">{file ? file.name : 'Selecionar XML RCTO'}</h3>
            <p className="text-xs text-gray-400 font-medium mt-1">{file ? 'Pronto para auditoria' : 'Arraste o arquivo XML do RCTO aqui'}</p>
          </div>
