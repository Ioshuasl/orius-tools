...
export function FloatingToolbar({ position, isVisible }: FloatingToolbarProps) {
  ...
  const applyFormat = (command: string, value?: string) => {
    // 1. O ideal aqui seria capturar o estado ANTES do comando
    // Mas como o comando é nativo, o handleInput do TextBlock cuidará do "Depois"
    document.execCommand(command, false, value);

    // 2. Força o TextBlock a disparar o onUpdate
    const activeElement = document.activeElement;
    if (activeElement) {
      activeElement.dispatchEvent(new Event('input', { bubbles: true }));
    }
