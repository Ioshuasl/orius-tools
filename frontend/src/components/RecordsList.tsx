...
export function RecordsList({ registros }: RecordsListProps) {
  ...
  const filteredData = useMemo(() => {
    // Garante que registros seja um array antes de filtrar
    return (registros || []).filter(reg => {
      const matchStatus = showOnlyErrors ? reg.status_registro !== 'OK' : true;
      
      // CORREÇÃO: Tratamento seguro para reg.codigo e reg.pedido (evita erro 'undefined')
      const pedidoStr = String(reg.pedido ?? '');
      const codigoStr = String(reg.codigo ?? '');
      const tipoAtoStr = (reg.tipo_ato || '').toLowerCase();
      const term = searchTerm.toLowerCase();

      const matchSearch = searchTerm === '' || 
        pedidoStr.includes(term) || 
        codigoStr.includes(term) || 
        tipoAtoStr.includes(term);
      
      return matchStatus && matchSearch;
    });
  }, [registros, showOnlyErrors, searchTerm]);

  const formatarValor = (campo: string, valor: number | null | undefined) => {
    ...
    <div className="space-y-4 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 transition-colors duration-200">
        ...
        <div className="flex w-full sm:w-auto items-center gap-3">
          <div className="relative w-full sm:w-64">
            ...
            <input 
              ...
              className="w-full pl-9 pr-4 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 outline-none transition-all"
            />
          </div>
          <button
      ...
      <div className="space-y-3 mt-4">
        {filteredData.map((reg, index) => {
            const uniqueKey = `${reg.pedido}-${reg.codigo ?? 'na'}-${index}`;
            const isExpanded = expandedId === uniqueKey;
            const hasError = reg.status_registro !== 'OK';

            return (
              <div key={uniqueKey} className={`bg-white dark:bg-gray-800 rounded-xl border transition-all ${
                  hasError ? 'border-l-4 border-l-red-500 border-gray-200 dark:border-gray-700' : 'border-l-4 border-l-green-400 border-gray-200 dark:border-gray-700'
                }`}
              >
                ...
                >
                  <div className="flex items-start md:items-center gap-4 flex-1">
                    <div className={`p-2 rounded-full shrink-0 ${hasError ? 'bg-red-50 dark:bg-red-500/20 text-red-600' : 'bg-green-50 dark:bg-green-500/20 text-green-600'}`}>
                    ...
                    <div className="flex-1 space-y-1">
                      <div className="flex items-center gap-2 text-xs font-semibold text-gray-500">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          >
                            {copiedId === reg.pedido ? <CheckCircle2 size={12} className="text-green-600"/> : <Copy size={12} />}
                          </button>
                        </div>
                      </div>
                      <h4 className="font-medium text-gray-900 dark:text-gray-100 text-sm md:text-base leading-snug">
                        {reg.tipo_ato}
                      </h4>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between md:justify-end gap-4 md:w-auto border-t md:border-t-0 pt-3 md:pt-0 mt-2 md:mt-0 border-gray-100 dark:border-gray-700">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${
                      hasError ? 'bg-red-100 text-red-700 dark:bg-red-500/20' : 'bg-green-100 text-green-700 dark:bg-green-500/20'
                    }`}>
                ...
                {isExpanded && (
                  <div className="border-t border-gray-100 dark:border-gray-700 bg-gray-50/80 dark:bg-gray-900/50 p-5 rounded-b-xl animate-in slide-in-from-top-2 duration-200">
                    {/* Caso o registro não tenha detalhes (ex: Não encontrado no PDF) */}
                    {!reg.detalhes || reg.detalhes.length === 0 ? (
                      <p className="text-sm text-gray-500 text-center italic">Não há detalhamento disponível para este status.</p>
                    ) : (
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        {reg.detalhes
                          ...
                          <div key={idx} className={`p-3 rounded-lg border text-sm bg-white dark:bg-gray-800 ${
                              detalhe.status === 'DIVERGENTE' ? 'border-red-200 dark:border-red-500/30' : 'border-gray-200 dark:border-gray-700'
                            }`}
                          >
                            ...
                            <div className="space-y-1">
                              <div className="flex justify-between gap-4">
                                <span className="text-gray-400 text-xs">Sistema (PDF):</span>
                                <strong className="text-gray-700 dark:text-gray-200">{formatarValor(detalhe.campo, detalhe.valor_sistema)}</strong>
                              </div>
                              <div className="flex justify-between gap-4">
                                <span className="text-gray-400 text-xs">Arquivo (CSV):</span>
                              ...
                              {detalhe.status === 'DIVERGENTE' && (
                                <div className="flex justify-between pt-2 mt-1 border-t border-red-50 dark:border-red-500/20">
                                  <span className="text-red-400 text-xs font-semibold">Diferença:</span>
        ...
        {filteredData.length === 0 && (
          <div className="text-center py-16 px-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-700 text-gray-400">
            <Search size={40} className="mx-auto mb-3 opacity-20" />
            <p className="font-medium">Nenhum registro encontrado.</p>
            <p className="text-sm">Tente limpar a busca ou alterar o filtro para ver todos os atos.</p>
          </div>
        )}
      </div>
    </div>
