...
export default function EditorComunidade() {
  ...
  const saveHistory = useCallback((state: CommunityPage, immediate = true) => {
    ...
    const performSave = () => {
      setPast(prev => [...prev.slice(-(HISTORY_LIMIT - 1)), JSON.parse(lastSnapshotRef.current || stateString)]);
      setFuture([]);
    ...
    if (immediate) {
      ...
      if (snapshotTimerRef.current) clearTimeout(snapshotTimerRef.current);
      // Agrupa mudanças de texto (espera 1.5s de silêncio)
  ...
  const updateBlock = useCallback((bId: string, newData: any, immediate = false) => {
    ...
    setPage(prev => {
      ...
      const newContent = prev.content.map(b =>
        b.id === bId ? { ...b, data: { ...b.data, ...newData } } : b
      );
  ...
  // Dentro de EditorComunidade.tsx

  const handlePaste = useCallback(async (e: React.ClipboardEvent) => {
    // 1. Tenta extrair arquivos do clipboard usando o helper
    const clipboardFiles = getFilesFromClipboard(e);

    // --- LÓGICA PARA ARQUIVOS (Imagens, Vídeos, PDFs) ---
    if (clipboardFiles.length > 0) {
      ...
      for (const item of clipboardFiles) {
        ...
        try {
          // Gerar o preview se for PDF antes do upload para ter os dados prontos
          let pdfThumbnail: string | null = null;
          if (item.file.type === 'application/pdf') {
            pdfThumbnail = await generatePdfThumbnail(item.file);
          }

          // Realiza o upload para o servidor
          const response = await uploadMediaService(item.file);

          if (response.success && response.url) {
            const newBlockId = Math.random().toString(36).substring(2, 11);

            // Criar o objeto do bloco já com todas as informações
            const newBlock: Block = {
              id: newBlockId,
              type: item.type,
              data: {
                url: response.url,
                filename: response.filename || item.file.name,
                mimetype: response.mimetype || item.file.type,
                size: item.file.size,
                thumbnail: pdfThumbnail // Insere o preview gerado localmente
              }
            };

            // Insere o bloco no estado da página
            setPage(prev => {
              if (!prev) return null;
            ...
            setHasUnsavedChanges(true); // Marca que há alterações para o autosave
            setFocusBlockId(newBlockId);
            toast.success(`${item.type.toUpperCase()} enviado com sucesso!`, { id: toastId });
          }
        } catch (error) {
          console.error("Erro no upload via paste:", error);
          toast.error("Falha ao processar arquivo colado.", { id: toastId });
        }
      }
      return; // Interrompe para não processar como texto
    }

    // --- LÓGICA PARA TEXTO / MARKDOWN ---
    const pastedText = e.clipboardData.getData('text/plain');

    // Verifica se contém marcadores de markdown ou múltiplas linhas
    if (/^[#*>-]|^\d+\.|```/m.test(pastedText) || pastedText.includes('\n')) {
      e.preventDefault();
      const newBlocks = parseMarkdownToBlocks(pastedText); // Converte markdown para blocos
      ...
      setPage(prev => {
        ...
        // Se o bloco onde estou colando está vazio, substituo ele pelos novos
        const isCurrentEmpty = currentBlock?.type === 'text' && !currentBlock.data.text;

        if (isCurrentEmpty) {
          newContent.splice(insertIndex, 1, ...newBlocks);
        } else {
          newContent.splice(insertIndex + 1, 0, ...newBlocks);
        }
      ...
      setHasUnsavedChanges(true);
      // Move o foco para o último bloco inserido
      setFocusBlockId(newBlocks[newBlocks.length - 1].id);
    }
  }, [focusBlockId, setPage, setHasUnsavedChanges, setFocusBlockId]);

  const handlePasteMarkdown = useCallback((e: React.ClipboardEvent) => {
    const pastedText = e.clipboardData.getData('text/plain');

    // Se contiver marcadores de markdown ou múltiplas linhas
    if (/^[#*>-]|^\d+\.|```/m.test(pastedText) || pastedText.includes('\n')) {
      e.preventDefault();
  ...
  const undo = useCallback(() => {
    ...
    setFuture(f => [page!, ...f].slice(0, HISTORY_LIMIT));
    setPage(previous);
    setPast(newPast);
  ...
  const redo = useCallback(() => {
    ...
    setPast(p => [...p, page!].slice(-(HISTORY_LIMIT - 1)));
    setPage(next);
    setFuture(newFuture);
  ...
  const takeSnapshot = useCallback(() => {
    if (page) {
      setPast(prev => [...prev, JSON.parse(JSON.stringify(page))].slice(-HISTORY_LIMIT));
      setFuture([]); // Sempre que uma nova ação é feita, o futuro é limpo
    }
  }, [page]);

  // Listener global para os atalhos Ctrl+Z e Ctrl+Y/Shift+Z
  useEffect(() => {
      ...
      <main ref={mainScrollRef} onPaste={handlePaste} className="flex-1 flex flex-col overflow-y-auto scroll-smooth">
        ...
        <div className="max-w-4xl mx-auto w-full py-12 px-12 pb-60 relative">
          <input
            value={page?.title || ''}
            onChange={(e) => {
              if (page) saveHistory(page, false);
              setPage(p => p ? { ...p, title: e.target.value } : null);
              setHasUnsavedChanges(true);
            }}
