...
import type { ApiResponseCensec } from '../types';
...
export default function Doi() {
  ...
  const processFile = (selectedFile: File) => {
    if (!selectedFile.name.endsWith('.json') && !selectedFile.name.endsWith('.JSON')) {
          ...
          {result.success ? (
            <div className="space-y-6 animate-in zoom-in-95 duration-500">
              <div className="bg-white dark:bg-gray-800 p-10 rounded-3xl border-2 border-dashed border-green-200 dark:border-green-900/30 flex flex-col items-center justify-center text-center shadow-sm">
                ...
                {result.foi_higienizado ? (
                  <>
                    <p className="text-orange-500 max-w-sm mt-2 text-sm font-bold">
                      O arquivo foi higienizado automaticamente (ajustes de sintaxe e hífens no CIB).
                    </p>
                    <button
                      onClick={() => {
                        const blob = new Blob([JSON.stringify(result.conteudo_limpo, null, 2)], { type: 'application/json' });
                      ...
                      className="mt-6 px-8 py-3 bg-orange-500 text-white rounded-xl font-bold text-xs shadow-lg flex items-center gap-2 hover:bg-orange-600 transition-all"
                    >
                      <Download size={16} /> Baixar JSON Corrigido
                    </button>
                  </>
                ) : (
                  <p className="text-gray-500 max-w-sm mt-2 text-sm font-medium">O lote está 100% íntegro conforme o manual da Receita Federal.</p>
                )}
